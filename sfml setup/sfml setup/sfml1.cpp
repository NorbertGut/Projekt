...
private:
	...
	void rysujNakladkePomocy();
	void rysujNakladkeWyjscia();
...
void Symulacja::renderuj() {
	...
	if (pokazPomoc) rysujNakladkePomocy();
	if (czyWyjsciePotwierdzone) rysujNakladkeWyjscia();

	okno.display();
}

void Symulacja::rysujNakladkePomocy() {
...
void Symulacja::rysujNakladkeWyjscia() {
	if (czcionka.getInfo().family.empty()) return;
...
void Symulacja::rysujWidokUproszczony() {
	if (czcionka.getInfo().family.empty()) return;

	sf::RectangleShape bg(sf::Vector2f(500.f, 180.f));
	bg.setFillColor(sf::Color(0, 0, 0, 200));
	float bx = (okno.getSize().x - 500.f) * 0.5f;
	float by = (okno.getSize().y - 180.f) * 0.5f;
	...
	oss << "UPROSZCZONY WIDOK\n\n"
		<< "Aktualna temperatura boilera: " << (int)zbior2.temperatura << "C,  docelowo: 90C\n"
		<< "Aktualna temperatura CO: " << (int)zbior3.temperatura << "C   Zadana: " << (int)temperaturaZadana3 << "C\n"
		<< "Aktualna temperatura CWU: " << (int)zbior4.temperatura << "C  Zadana: " << (int)temperaturaZadana4 << "C\n";
